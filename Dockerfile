...
RUN \
    # Get EJAM R package (v2.32.6.002)
    wget -c https://github.com/ejanalysis/EJAM/archive/refs/tags/v2.32.6.002.tar.gz -O - | tar -xz && \
    \
    ...
    MAKEFLAGS="-j1" R -e " \
        ...
        remotes::install_local('/EJAM-2.32.6.002', dependencies=TRUE, upgrade='always', build=FALSE, repos=c('https://packagemanager.rstudio.com/all/__linux__/jammy/latest'), INSTALL_opts=c('--preclean', '--no-multiarch', '--with-keep.source')) \
    " && \
    \
    # Clean up the downloaded source directory
    rm -rf /EJAM-2.32.6.002
